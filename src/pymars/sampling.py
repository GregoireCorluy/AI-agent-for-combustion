...
#from .simulation import Simulation, Simulation_PSR, Simulation_PLFlame
...
class InputPSR(NamedTuple):
    ...
    reactants: Dict = {}
    composition_type: str = 'mole'
    target_species_PSR: set = {} 



class InputPLFlame(NamedTuple):
    """Holds input parameters for single premixed laminar flame simulation.
    """
    #kind: str
    temperature: float
    pressure: float
...
def simulation_worker_plflame(sim_tuple):
    """Worker for multiprocessing of simulation cases.
    ...
    sim = Simulation_PLFlame(sim.idx, sim.properties, sim.model, phase_name=sim.phase_name, path=sim.path)

    return sim

def simulation_worker_psr(sim_tuple):
    """Worker for multiprocessing of simulation cases.
...
def psr_worker(sim_tuple):
    ...
    sim, idx = sim_tuple
    sim.setup_case()
    psr_X = sim.calculate_psr()
    
    #print('sim psr worker:     ', type(sim), vars(sim))

    #print('sim psr worker:     ', sim.gas.X)
...
def plflame_worker(sim_tuple):
    ...
    sim, idx = sim_tuple
    sim.setup_case()
    plflame_lbv = sim.calculate_plflame()
...
def calculate_error_psr(metrics_original, metrics_test):
    ...
    #print('metrics_original:   ', metrics_original)

    #print('metrics_test:   ', metrics_test)
    #print('metrics_originallllll       ',metrics_original)
    #print('metrics_originallllll[0]       ',metrics_original[0])

    N_species = len(metrics_original[0])
    error_vec = np.zeros(len(metrics_original))
    #print('metrics:     ', metrics_original, metrics_test)
    for i in range(len(error_vec)):
         #print('metrics: ', np.abs(metrics_original[i] - metrics_test[i])/metrics_original[i] *100 )
         error_vec[i] = np.max( ( np.abs(metrics_original[i] - metrics_test[i])/metrics_original[i] ) ) #max entre todas as especies

    #print('error_vec: ', error_vec*100)
    error = 100 *  np.max(   error_vec ) 
...
def calculate_error_plflame(metrics_original, metrics_test):
    ...
    #print('mtc or:      ', metrics_original)
    #print('mtc tst:      ', metrics_test)
...
def sample_metrics(model, ignition_conditions=[], psr_conditions=[], plflame_conditions=[],
...
# |___|____/ |_|  
    ...
    if ignition_conditions:
        ...
        if reuse_saved and exists_output:
            ignition_delays = np.genfromtxt(data_files['output_ignition'], delimiter=',', ndmin=1)
            #print('ig dl:    ', ignition_delays)
            #print('len ig dl:    ', len(ignition_delays))
...
# |_|   |____/|_| \_\
    ...
    if psr_conditions:
            ...
            #if reuse_saved and exists_output:
            #    X_psr_aux = np.genfromtxt(data_files['output_psr'], delimiter=',')
             ...
             #   logging.info('Reusing existing psr samples for the starting model.')
                ...
                results = {key:val for k in results for key, val in k.items()}
                #ignition_delays = np.zeros(len(results))
                for idx, psr_data_aux in results.items():
                    X_psr_aux.append(psr_data_aux)
                
                X_psr_aux = np.array(X_psr_aux)
            #return X_psr_aux   
    ...
    if plflame_conditions:
            ...
            #    X_psr_aux = np.genfromtxt(data_files['output_psr'], delimiter=',')
             ...
             #   logging.info('Reusing existing psr samples for the starting model.')
                ...
                simulations = []
                plflame_lbv_aux=[]
                for idx, case in enumerate(plflame_conditions):
                    simulations.append([
                        Simulation_PLFlame(idx, case, model, phase_name=phase_name, path=path), idx])
    
                jobs = tuple(simulations)
                if num_threads == 1:
                    results = []
                    for job in jobs:
                        results.append(plflame_worker(job))
                else:
                    pool = multiprocessing.Pool(processes=num_threads)
                    results = pool.map(plflame_worker, jobs)
                    pool.close()
                    pool.join()
    
                results = {key:val for k in results for key, val in k.items()}
                #ignition_delays = np.zeros(len(results))
                for idx, plflame_data_aux in results.items():
                    plflame_lbv_aux.append(plflame_data_aux)
                
                plflame_lbv_aux = np.array(plflame_lbv_aux)
            #return X_psr_aux   
        

    return ignition_delays, X_psr_aux, plflame_lbv_aux
...
def parse_psr_inputs(model, conditions, phase_name=''):
    ...
    for idx, case in enumerate(conditions):
        ...
        temperature = case.get('temperature', 0.0)
        pressure = case.get('pressure', 0.0)
        species_PSR = case.get('target_species_PSR', [])
        #assert kind in ['constant volume', 'constant pressure'], (
        #    pre + '"case" needs to be "constant volume" or "constant pressure'
        #    )
        assert temperature > 0.0, pre + '"temperature" needs to be > 0'
        ...
        end_time = case.get('end-time', 60)
        #max_steps = case.get('max-steps', 10000)
        residence_time = case.get('residence_time', 1)
        equiv_ratio = case.get('equivalence-ratio', 0.0)
        fuel = case.get('fuel', [])
        oxidizer = case.get('oxidizer', [])
        ...
        inputs.append(InputPSR(
            temperature, pressure, end_time, residence_time, 
            equiv_ratio, fuel, oxidizer, reactants, composition_type, species_PSR
        ))
...
def parse_plflame_inputs(model, conditions, phase_name=''):
    """Parses input for premixed laminar flame simulations, raising an error on any errors.
    ...
    for idx, case in enumerate(conditions):
        ...
        #residence_time = case.get('residence_time', 1)
        equiv_ratio = case.get('equivalence-ratio', 0.0)
        #print('equiv_ratiooooooooo:     ', equiv_ratio)
        fuel = case.get('fuel', [])
        oxidizer = case.get('oxidizer', [])
...
def sample(model, ignition_conditions=[], psr_conditions=[], plflame_conditions=[], phase_name='', num_threads=1, path=''):
        ...
        if aux_skip_reuse==1:
         ...
         #   logging.info('Running autoignition simulations for starting model.')
            ...
            for idx, sim in enumerate(results):
                ...
                data_conc += list(data)
                sim.clean()
                #print(idx, len(results), len(data), len(ignition_data))

            ignition_data_aux = np.array(ignition_data)
            


            np.savetxt(data_files['data_ignition'], ignition_data_aux, delimiter=',')
    ...
    if psr_conditions:
        ...
        if exists_data and exists_output:
            psr = np.genfromtxt(data_files['output_psr'], delimiter=',')
            psr_data = np.genfromtxt(data_files['data_psr'], delimiter=',')
            # need to check that saved data at least matches the number of cases
            matches_number = (
                psr.size == len(psr_conditions) and 
        ...
        if matches_number and matches_shape:
            ...
            for idx, sim in enumerate(results):
                psr_X_arr_aux, data = sim.process_results()
                #print('process results:   ', psr_X_arr_aux, type(psr_X_arr_aux), data, type(data))
                psr_X_arr += list(psr_X_arr_aux)
                psr_data += list(data)
                data_conc+=list(data)
                sim.clean()
            
            psr_X_arr = np.array(psr_X_arr)
            psr_data_aux = np.array(psr_data)


            np.savetxt(data_files['data_psr'], psr_data_aux, delimiter=',')
            np.savetxt(data_files['output_psr'], psr_X_arr, delimiter=',')

        #return psr_X_arr, psr_data
     
    #ignition_data = np.array(ignition_data)
    #psr_data = np.array(psr_data)


    #print('data ig        :', ignition_data)
    #print('data psr        :', psr_data)
    ...
    if plflame_conditions:
        ...
        #print('entrou plflame conditionns')

        # check for presence of data and output files; if present, reuse.
        matches_number = False
        matches_shape = False
        exists_data = os.path.isfile(data_files['data_plflame'])
        exists_output = os.path.isfile(data_files['output_plflame'])
  
        # 'data_psr': 'psr_data.dat', 'output_psr': 'psr_output.txt',


        if exists_data and exists_output:
            plflame = np.genfromtxt(data_files['output_plflame'], delimiter=',')
            plflame_data = np.genfromtxt(data_files['data_plflame'], delimiter=',')
            # need to check that saved data at least matches the number of cases
            matches_number = (
                plflame.size == len(plflame_conditions) and 
                plflame_data.shape[0] / 20 == len(plflame_conditions)
                )
            
            # also check that expected data is right shape (e.g., in case number of species 
            # has changed if running a new model)
            gas = ct.Solution(model, phase_name)
            matches_shape = plflame_data.shape[1] == 2 + gas.n_species

        if matches_number and matches_shape:
            logging.info('Reusing existing plflame samples for the starting model.')
        else:
            logging.info('Running plflame simulations for starting model.')
            #stop_at_ignition = False
            simulations = []
            for idx, case in enumerate(plflame_conditions):
                simulations.append(
                    Simulation_PLFlame(idx, case, model, phase_name=phase_name, path=path)
                    )
            ...
            if num_threads == 1:
                results = []
                for job in jobs:
                    results.append(simulation_worker_plflame(job))
            else:
                pool = multiprocessing.Pool(processes=num_threads)
                results = pool.map(simulation_worker_plflame, jobs)
                pool.close()
                pool.join()
            ...
            for idx, sim in enumerate(results):
                plflame_lbv_aux, data = sim.process_results()
                #print('plflame_lbv_aux      ', plflame_lbv_aux)
                #print('process results:   ', psr_X_arr_aux, type(psr_X_arr_aux), data, type(data))
                plflame_lbv.append(plflame_lbv_aux)
                plflame_data += list(data)
                data_conc+=list(data)
                sim.clean()
            
            plflame_lbv = np.array(plflame_lbv)
            plflame_data_aux = np.array(plflame_data)


            np.savetxt(data_files['data_plflame'], plflame_data_aux, delimiter=',')
            np.savetxt(data_files['output_plflame'], plflame_lbv, delimiter=',')



    data_conc = np.array(data_conc)

    #print('plflame_lbv inside sample:      ', plflame_lbv)
    
    #print('data conc        :', data_conc)
    return (ignition_delays, psr_X_arr, plflame_lbv, data_conc)
