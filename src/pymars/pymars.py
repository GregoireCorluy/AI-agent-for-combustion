...
# local imports
#from .sampling import sample_metrics, parse_ignition_inputs, parse_psr_inputs, parse_plflame_inputs
#from .sampling import InputIgnition, InputPSR, InputPLFlame
...
class ReductionInputs(NamedTuple):
    ...
    plflame_conditions: List[InputPLFlame]
    method: str
...
def parse_inputs(input_dict):
    ...
    plflame_conditions = input_dict.get('premixed-laminar-flame-conditions', {})
    #if psr_conditions:
    ...
    plflame_inputs = parse_plflame_inputs(model, plflame_conditions, phase_name)

    #print('safe species inside parse_inputs: ', safe_species)
    #print('target species inside parse_inputs: ', target_species)

    
    return ReductionInputs(
        model=model, error=error, 
        ignition_conditions=ignition_inputs, 
        psr_conditions=psr_inputs, plflame_conditions=plflame_inputs,
...
def main(model_file, error_limit, 
    ...
    if method == 'DRG':
        reduced_model = run_drg(
            model_file, ignition_conditions, psr_conditions, plflame_conditions,
    ...
    if method in ['DRG', 'DRGEP', 'PFA']:
        ...
        error_idt = reduced_model.error_idt
        error_psr = reduced_model.error_psr
        error_plflame = reduced_model.error_plflame
    ...
    if run_sensitivity_analysis:
        ...
        reduced_model = run_sa(
            model_file, error_idt, error_psr, error_plflame, ignition_conditions, psr_conditions, plflame_conditions, 
            error_limit, target_species + safe_species, phase_name=phase_name,
...
def pymars(argv):
    ...
    if args.convert:
        ...
        print(inputs.model)
        #print('!!!!!', os.path.splitext(inputs.model)[1])
        ...
        main(
            ...
            inputs.ignition_conditions, inputs.psr_conditions, inputs.plflame_conditions,
