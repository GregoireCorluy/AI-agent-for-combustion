...
def reduce_pfa(model_file, species_targets, species_safe, threshold, 
               matrices, ignition_conditions, psr_conditions, plflame_conditions,
               sampled_metrics_idt, sampled_metrics_psr, sampled_metrics_plflame, phase_name='',
    ...
    #print(' species_safe inside pfa: ',  species_safe)   
    #print('species_retained + species_safe', species_retained + species_safe)
    ...
    reduced_model.write_yaml(f'reduced_{reduced_model.n_species}.yaml')
    ...
    return ReducedModel(
        ...
        error_idt=error_idt, error_psr=error_psr, error_plflame=error_plflame, limbo_species=limbo_species
        )
    


def run_pfa(model_file, ignition_conditions, psr_conditions, plflame_conditions, 
    ...
    sampled_metrics_idt, sampled_metrics_psr, sampled_metrics_plflame, sampled_data = sample(
        model_file, ignition_conditions, psr_conditions, plflame_conditions, phase_name=phase_name, 
        num_threads=num_threads, path=path
        )
    ...
    while error_current_psr <= error_limit and error_current_idt <= error_limit  and error_current_plflame <= error_limit:
        
        reduced_model = reduce_pfa(
            model_file, species_targets, species_safe, threshold, matrices, 
            ignition_conditions, psr_conditions, plflame_conditions,
            sampled_metrics_idt, sampled_metrics_psr, sampled_metrics_plflame, phase_name=phase_name, 
        ...
        previous_model = ReducedModel(
            ...
            error_idt=reduced_model.error_idt, error_psr=reduced_model.error_psr, error_plflame=reduced_model.error_plflame,
            limbo_species=reduced_model.limbo_species
            )
    
    if reduced_model.error_idt > error_limit or reduced_model.error_psr > error_limit or reduced_model.error_plflame > error_limit:
        threshold -= (2 * threshold_increment)
        reduced_model = reduce_pfa(
            model_file, species_targets, species_safe, threshold, matrices, 
            ignition_conditions, psr_conditions, plflame_conditions, sampled_metrics_idt, sampled_metrics_psr,
            sampled_metrics_plflame, phase_name=phase_name,
            threshold_upper=threshold_upper, num_threads=num_threads, path=path
            )
    else:
