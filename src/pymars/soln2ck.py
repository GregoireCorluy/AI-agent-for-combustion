...
def build_arrhenius(rate, reaction_order, reaction_type):
    ...
    elif reaction_type in ['falloff-Troe', 'falloff-Lindemann', 'chemically-activated']:
...
def build_falloff_arrhenius(rate, reaction_order, reaction_type, pressure_limit):
    ...
    if reaction_type in ['falloff-Troe' , 'falloff-Lindemann']:
        if pressure_limit == 'low':
            pre_exponential_factor = rate.pre_exponential_factor * 1e3**(reaction_order)
        elif pressure_limit == 'high':
            pre_exponential_factor = rate.pre_exponential_factor * 1e3**(reaction_order - 1)

    elif reaction_type == 'chemically-activated':
        if pressure_limit == 'low':
            pre_exponential_factor = rate.pre_exponential_factor * 1e3**(reaction_order - 1)
        elif pressure_limit == 'high':
...
def build_falloff(parameters, falloff_function):
    ...
    if parameters.size < 4:
        parameters = np.append(parameters, 0.0, axis=None)
    #print(parameters, type(parameters), parameters.size)
    ...
    elif falloff_function == 'Lindemann':
        falloff_string = ('')
...
def write(solution, output_filename='', path='', 
    ...
    with open(output_filename, 'w') as the_file:
        ...
        for reaction in solution.reactions():
            ...
            #print(reaction, reaction.reaction_type, i_aux)

            if reaction.reaction_type in ['Arrhenius', 'three-body-Arrhenius']:
            ...
            elif reaction.reaction_type == 'falloff-Troe':
            ...
            elif reaction.reaction_type == 'falloff-Lindemann':
                # high-pressure limit is included on the main reaction line
                arrhenius = build_falloff_arrhenius(
                    reaction.rate.high_rate, 
                    sum(reaction.reactants.values()), 
            ...
            elif reaction.reaction_type == 'chemically-activated':
                ...
                #print(reaction, i_aux, reaction.reaction_type)
                raise NotImplementedError(f'Unsupported reaction type: {reaction.reaction_type}')

            reaction_string += arrhenius + '\n'
            
            # need to trim and print third-body efficiencies, if present
            if reaction.reaction_type in ['three-body-Arrhenius', 'falloff-Troe', 'falloff-Lindemann', 
                                  'chemically-activated'
                                  ]:
                # trims efficiencies list
                reduced_efficiencies = {s:reaction.third_body.efficiencies[s] 
                                        for s in reaction.third_body.efficiencies
            ...
            if reaction.reaction_type == 'falloff-Troe':
                ...
                if reaction.rate.falloff_coeffs.size > 0:
                    #print(reaction, i_aux)
                    falloff_str = build_falloff(reaction.rate.falloff_coeffs, reaction.rate.sub_type)
